...
from utils.rope import _precompute_rope_cache, _precompute_rope_cache_llama3
...
class LlamaConfig(BaseConfig):
    ...
    hidden_act: str = "silu"
    qk_norm: bool = False
    attention_bias: bool = False # Llama does not use attention bias
    rope_scaling: Optional[dict] = field(default=None)

    @classmethod
    def from_hf_config(cls, config_dict: dict) -> "LlamaConfig":
        if config_dict.get("attention_bias", False):
            # As requested, we don't support attention bias for Llama
            raise NotImplementedError("OpenFormer does not support Llama models with attention_bias=True.")
...
class LlamaModel(BaseModel):
    def __init__(self, config: LlamaConfig, linear_class: Type):
        ...
        # Select the correct RoPE precomputation function based on the config
        if config.rope_scaling and config.rope_scaling.get("rope_type") == "llama3":
            print("Using Llama 3 specific RoPE scaling.")
            config.rope_scaling['rope_theta'] = config.rope_theta # Add theta to the dict for the func
