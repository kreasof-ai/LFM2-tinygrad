...
class Gemma3Config(BaseConfig):
    """Configuration class for Gemma 3."""
    head_dim: int = 256
    hidden_act: str = "gelu" # Gemma3 uses gelu_pytorch_tanh, which is tinygrad's gelu
    qk_norm: bool = True # Custom flag, though it uses a different norm
...
# --- Model Components (Gemma3 Specific) ---

class Gemma3RMSNorm:
    """ Custom RMSNorm for Gemma3 with (1 + weight) scaling. """
...
class Gemma3MLP:
    """ Gemma3's MLP uses GELU activation. """
    def __init__(self, config: Gemma3Config, linear_class: Type = Linear):
...
class Gemma3Attention:
    """
    Self-contained attention module for Gemma3. Does not use BaseAttention because it requires a custom
    attention scaling factor not supported by tinygrad's SDPA.
    """
    def __init__(self, config: Gemma3Config, linear_class: Type):
        self.config = config
...
class Gemma3DecoderLayer:
    def __init__(self, config: Gemma3Config, layer_idx: int, linear_class: Type):
        self.self_attn = Gemma3Attention(config, linear_class)
        self.mlp = Gemma3MLP(config, linear_class)
...
class Gemma3Model:
    def __init__(self, config: Gemma3Config, linear_class: Type):
        ...
        self.layers = [Gemma3DecoderLayer(config, i, linear_class) for i in range(config.num_hidden_layers)]
        self.norm = Gemma3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        
        self.cos_cache_full, self.sin_cache_full = _precompute_rope_cache(dim=config.head_dim, max_seq_len=config.max_position_embeddings, base=config.rope_theta, dtype=config.dtype)
        self.cos_cache, self.sin_cache = self.cos_cache_full, self.sin_cache_full # alias for debugging
        self.cos_cache_sliding, self.sin_cache_sliding = _precompute_rope_cache(dim=config.head_dim, max_seq_len=config.max_position_embeddings, base=config.rope_local_base_freq, dtype=config.dtype)
    ...
    def __call__(self, input_ids: Tensor, past_states: Optional[List[Any]], start_pos: int, output_hidden_states: bool, **kwargs):
        ...
        causal_mask = Tensor.full((1, 1, seq_len, seq_len), -float("inf")).triu(1) if seq_len > 1 else None
        sliding_mask = None
        if self.config.sliding_window and seq_len > 1:
            ...
            sliding_mask = (causal_mask + sliding_part).reshape(1, 1, seq_len, seq_len)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if layer_type == "full_attention":
                mask = causal_mask
                cos = self.cos_cache_full[start_pos : start_pos + seq_len]
                sin = self.sin_cache_full[start_pos : start_pos + seq_len]
            else: # sliding_attention
                mask = sliding_mask if sliding_mask is not None else causal_mask
                cos = self.cos_cache_sliding[start_pos : start_pos + seq_len]
                sin = self.sin_cache_sliding[start_pos : start_pos + seq_len]

            cos = cos.reshape(1, 1, seq_len, self.config.head_dim).expand(bsz, 1, -1, -1)
            sin = sin.reshape(1, 1, seq_len, self.config.head_dim).expand(bsz, 1, -1, -1)
