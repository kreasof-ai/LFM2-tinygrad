...
class Qwen2Config(BaseConfig):
    """Configuration class for Qwen2."""
    head_dim: int = 128 # This will be overridden from config
    hidden_act: str = "silu"
    qk_norm: bool = False # Qwen2 does not use QK Norm
    attention_bias: bool = True # Qwen2 uses bias in q, k, v projections

    @classmethod
    def from_hf_config(cls, config_dict: dict) -> "Qwen2Config":
        # Qwen2 doesn't have head_dim in its config, it must be calculated
...
class Qwen2DecoderLayer:
    def __init__(self, config: Qwen2Config, linear_class: Type):
    ...
    def _get_key_map(self) -> dict:
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            key_map.update({
                ...
                f"{p}.self_attn.q_proj.bias": f"{p}.self_attn.q_proj.bias",
                f"{p}.self_attn.k_proj.bias": f"{p}.self_attn.k_proj.bias",
                f"{p}.self_attn.v_proj.bias": f"{p}.self_attn.v_proj.bias",
            })
        return key_map